...
class _ImportExcelScreenState extends State<ImportExcelScreen> {
  ...
  Future<void> _importData() async {
    ...
    try {
      ...
      // Fetch categories and payment methods for mapping
      ...
      for (var item in _previewData) {
        ...
        final category = categories.firstWhere(
          ...
          orElse: () =>
              categories.first, // Default to first category if not found
        );

        // Map Payment Method Name to ID
        final paymentMethod = paymentMethods.firstWhere(
          (p) =>
              p.name.toLowerCase() ==
              item['paymentMethodName'].toString().toLowerCase(),
          orElse: () =>
              paymentMethods.first, // Default to first method if not found
        );
